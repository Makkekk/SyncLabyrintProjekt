...
public class GUI extends Application {
	...
	public void start(Stage primaryStage) {
		try {
			...
			for (int j=0; j<20; j++) {
				for (int i=0; i<20; i++) {
					switch (TCPServer.board[j].charAt(i)) {
						case 'w':
						...
						case ' ':
							...
							break;
						default:
							throw new Exception("Illegal field value: " + TCPServer.board[j].charAt(i));
					}
    ...
    public static void updatePlayer(String name, int x, int y, String direction, int points) {
			...
			if (me == null && name.equals(player.name))
				me = player;
